...
apt upgrade -y
# reboot!
apt install -y python3-all python3-pip build-essential python3.8-venv libcurl4-openssl-dev screen
...
# APPLY THE PATCH NOW
#
# copy the patch to root of build, then:
# patch < thepatch.patch

# still illegal instruction!
#time python buildscripts/scons.py -j${CORES} --disable-warnings-as-errors --ssl CC=/usr/bin/aarch64-linux-gnu-gcc-${compiler_version} CXX=/usr/bin/aarch64-linux-gnu-g++-${compiler_version} CCFLAGS="-march=armv8-a+crc -mtune=cortex-a72" --install-mode=hygienic --install-action=hardlink --separate-debug archive-core{,-debug}
...
cp README mongodb.cd.pi.r4.4.26 mongodb.cd.pi.r4.4.26
cp APACHE-2.0.txt mongodb.cd.pi.r4.4.26
cp CreativeCommons.txt mongodb.cd.pi.r4.4.26
